document.addEventListener('DOMContentLoaded', () => {
  ...
  const instagramToggle = document.getElementById('instagram');
  ...
  chrome.storage.local.get({ isActive: true, fbReels: true, instagram: true, ytHome: true }, (data) => {
    isActiveToggle.checked = data.isActive;
    fbReelsToggle.checked = data.fbReels;
    instagramToggle.checked = data.instagram;
  ...
  instagramToggle.addEventListener('change', (e) => {
    chrome.storage.local.set({ instagram: e.target.checked });
  });
